...
public class SignSoups implements Listener
{
    @EventHandler
    ...
    public void inv(final PlayerInteractEvent e) {
        ...
        final Inventory inve = Bukkit.getServer().createInventory((InventoryHolder)p, 36, "§aSopas");
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WALL_SIGN || e.getClickedBlock().getType() == Material.SIGN_POST)) {
            ...
            if (lines.length > 0 && lines[0].equals("§8=-=-=-=-=") && lines.length > 1 && lines[1].equals("§6§lBLATANT") && lines.length > 2 && lines[2].equals("§a§lSOPAS") && lines.length > 3 && lines[3].equals("§8=-=-=-=-=")) {

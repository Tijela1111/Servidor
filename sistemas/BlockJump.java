...
public class BlockJump implements Listener
{
    ...
    static {
        BlockJump.naotomardanoqueda = new ArrayList<String>();
    }
    ...
    public void aoBlocoJump(final PlayerMoveEvent e) {
        ...
        if (e.getTo().getBlock().getRelative(BlockFace.DOWN).getType() == Material.SPONGE) {
            BlockJump.naotomardanoqueda.add(p.getName());
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, (Runnable)new Runnable() {
                @Override
                public void run() {
                    BlockJump.naotomardanoqueda.remove(p.getName());
                }
            }, 100L);
        }
    }
    ...
    public void aoCair(final EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            ...
            if (e.getCause().equals((Object)EntityDamageEvent.DamageCause.FALL) && BlockJump.naotomardanoqueda.contains(p.getName())) {
                e.setCancelled(true);
            }
        }
    }
    ...
    public void aoMorrer(final PlayerDeathEvent e) {
        ...
        if (BlockJump.naotomardanoqueda.contains(p.getName())) {
            BlockJump.naotomardanoqueda.remove(p.getName());
        }
    }

...
public class GuiCaixa implements Listener
{
    ...
    public static void GuiCaixas(final Player p) {
        final Inventory inv = Bukkit.createInventory((InventoryHolder)null, 9, "§aSorteador");
    ...
    private static void SortearKit(final Inventory inv, final Player p, final String kit, final Material mat) {
        ...
        itemm.setDisplayName("§a§lKIT §f" + GuiCaixa.kitsorteado.get(p.getName()));
        item.setItemMeta(itemm);
        if (!p.hasPermission("kit." + kit)) {
            PermissionsEx.getUser(p.getName()).addPermission("kit." + kit);
            p.sendMessage("§a§lKIT §fVocê ganhou o kit: §a" + GuiCaixa.kitsorteado.get(p.getName()));
            inv.setItem(4, item);
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.getInstance(), (Runnable)new Runnable() {
                ...
                public void run() {
                    p.closeInventory();
                    GuisEvent.aocaixa.remove(p.getName());
                }
            }, 65L);
        }
        else {
    ...
    public void aoMecher(final InventoryClickEvent e) {
        if (e.getInventory().getTitle().equalsIgnoreCase("§aSorteador") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
        }
    }
}

...
public class TpCMD implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String cmd, String[] args) {
			if (!(sender instanceof Player)) {
				...
				if (sender.hasPermission("simulator.tp")) {
					if (args.length == 0) {
						sender.sendMessage("§c§lTP §fUtilize: /tp (jogador) ou /tp (jogador) (alvo)");
					} else if (args.length == 1) {
						if (sender.getServer().getPlayer(args[0]) != null) {
							((Entity) sender).teleport(sender.getServer().getPlayer(args[0]).getLocation());
							sender.sendMessage("§a§lTP §fVocê foi teleportado para o jogador §a" + sender.getServer().getPlayer(args[0]).getName() + "!");
						} else {
							sender.sendMessage("§c§lTP §fO jogador está offline!");
						}
						...
						if ((x2 > 30000000.0D) || (y2 > 30000000.0D) || (z2 > 30000000.0D) || (x2 > -30000000.0D)
							...
							sender.sendMessage("§c§lTP §fUtilize: /tp (jogador) (alvo)");
							return true;
						}

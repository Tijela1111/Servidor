    ...
    public void OnGladiatorKit(final PlayerInteractEntityEvent event) {
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && Hability.getAbility(p).equalsIgnoreCase("Gladiator")) {
                ...
                if (Infernor.lutando.containsKey(p.getName()) || Infernor.lutando.containsKey(r.getName()) || Gladiator.lutando.containsKey(p.getName()) || Gladiator.lutando.containsKey(r.getName())) {
                    event.setCancelled(true);
                    p.sendMessage("§c§lGLADIATOR §fVocê já está em combate!");
                    return;
                }
